...
export class EditPhotoProfileComponent {
  ...
  ngAfterViewInit(): void {
    ...
    }, 100); // Retraso corto para asegurar que el DOM esté listo
  }
    ...
    this.dropzoneInstance = new Dropzone('#dropzone', {
      ...
      init: function () {
         ...
         if (dropzoneInstance.files.length > 1) {
          dropzoneInstance.files.forEach((f, index) => {
            if (index < dropzoneInstance.files.length - 1) {
              dropzoneInstance.removeFile(f); // Eliminar todos los archivos excepto el actual
            }
          });
        }
        ...
        this.on('success', (file, response) => {
          ...
          (document.getElementById('dropzone') as any).componentRef.showAlert('success', 'Imagen de perfil actualizada correctamente.');
      ...
      fetch(this.photoUrl)
        ...
        .then(blob => {
          ...
          this.profileService.updateProfile(formData).subscribe({
            next: (res) => {
              this.showAlert('success', 'Imagen de perfil actualizada correctamente.');
              this.close.emit();
            },
            error: (err) => {
              this.showAlert('error', 'Error al actualizar la imagen de perfil.');
              console.error(err);
            }
          });
        });
